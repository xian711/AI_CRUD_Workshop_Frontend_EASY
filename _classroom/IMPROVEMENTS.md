...
| **E4** | PRD 有 6 個必填，**只斷言了分類 1 個**——其餘漏寫驗證照樣綠 | 逐欄斷言 `categoryKey`／`name`／`unit`／`spec1` 四個欄位各自容器內都有非空錯誤訊息；另兩個必填（`qty`、`status`）因為有合法預設值不會觸發紅字，**改成明確驗預設值**（`qty` 為 0、`status` 顯示「正常」），不再默認<br>（此列為 v2 初版寫法，`qty` 與 `status` 的驗法在後續兩輪回修又各自加強了一次，見文末修正紀錄） |
...
> **後續：Codex 複驗這一輪判 F1／F3／runner 路徑 PASS，F2／F4 各留一個缺口——
> 請接著看最後的「v2 第二輪修正」。** 本節「六必填逐欄驗錯」的說法在第二輪已修正為
> 「五欄驗錯 ＋ 狀態進不了空值」。
...
- **修法**（`equipment.spec.ts` E4；測試名稱在下一輪回修又改過一次，見「v2 第二輪修正」）：
...
## v2 第二輪修正（Codex 複驗 FAIL 後）

> Codex 對 `93bc696` 複驗：**F1／F3／runner 路徑 PASS**，F2、F4 各留一個缺口，整體仍判 FAIL。
> 報告在 `D:\AIWORK\_AICLASS\_LAB\CODEX\workshop\_replies\v2_review2.md`。兩個缺口都照修。
> **回修後：`run-e2e.ps1` 與 `run-e2e.sh` 都是 7 passed / 0 failed / 0 skipped、exit 0。**

### F2 補件　狀態欄改採 invariant-by-construction，並把說法對齊

- **Codex 指出的缺口**：整支 E4 沒有 `fieldError(page, 'status')`。
  把參考解 `[id].vue` 的 `status: [{ required: true }]` 拿掉，E4 照樣全綠——
  等於第六個必填欄位其實**沒被驗到**，測試名稱卻宣稱「六個必填欄位逐欄把關」，那是不實陳述。
- **先確認事實再選路**：Codex 給了二擇一，我實際探了 UI 才決定：

  | 探測項目 | 結果 |
  |---|---|
  | 狀態欄內有幾顆 button | **1**（只有觸發鈕，沒有清除／叉叉） |
  | 選項全集 | **恰好 3 個**：正常／維修中／已報廢 |
  | 有沒有空白哨兵選項 | **沒有** |
  | 再點一次已選中的選項 | 值不變，**不會被取消成空值** |

  結論：**UI 上根本走不到「狀態為空」這個狀態**，所以「提供可清空路徑再斷言 fieldError」這條路
  在不改 App 的前提下做不到。硬湊一條 fieldError 只會變成另一種造假。
  → 採用 Codex 給的第二個選項：**明文 invariant-by-construction，並把說法改成實話。**

- **修法**（`equipment.spec.ts` E4）：
  1. **測試名稱改成**「E4 表單驗證：**五個必填逐欄驗錯 ＋ 狀態進不了空值**，錯誤不離頁」——
     不再宣稱六欄都經過錯誤訊息驗證。
  2. 測試上方新增註解，把兩種驗法講白：
     A 類（分類／項目／單位／規格說明／數量）＝使用者進得了不合法狀態 → 弄壞它、看錯誤訊息；
     B 類（狀態）＝使用者進不了空值 → 驗「不可能變成不合法」這件事本身。
  3. 新增第 (3) 段，對狀態實際斷言三件事：
     - `statusHost.locator('button')` **恰為 1 顆**（多一顆清除鈕就會紅）
     - 打開選單後 `getByRole('option')` **恰為 3 個**，且文字依序是 `正常／維修中／已報廢`
       （多一個空白哨兵就會紅）
     - **再點一次目前選中的那個**，狀態欄仍是「正常」（可切換成取消選取就會紅）
  4. 註解同時寫明**什麼時候該回來改**：哪天 UI 多了清除鈕或空白選項，3-1／3-2 會先紅，
     那就代表進得了空值了，應改成「清空 → 斷言 `fieldError(page, 'status')`」。
- **教材同步**（不能只改測試、不改對外說法）：
  - HANDBOOK 七條測試對照表的 E4 那一列改成「逐欄驗到 **5 個必填**；狀態沒有清空入口，
    改驗『選項只有三個合法值、沒有清除鈕』（進不了空值）」。
  - 本文件 v2-4 與第一輪 F2 兩處「六必填」的敘述都加註，指向這一節。

### F4 補件　shell 的 regex 補結尾錨點，並要求恰好兩欄

- **Codex 指出的缺口**：`run-e2e.sh:87` 的 regex 只錨開頭
  （`^[0-9a-fA-F]{64}[[:space:]]+[^[:space:]]`），後面又用 `awk '{print $2}'` 取欄位，
  所以這一行會被當成合法：

  ```
  <正確 hash>  e2e/tests/equipment.spec.ts EXTRA
  ```

  `EXTRA` 被靜靜丟掉，路徑仍解析成合法值 → 整個完整性檢查 PASS。
- **修法**（兩支腳本一起，避免只修一邊又長出不對稱）：
  - `run-e2e.sh`：regex 補結尾錨點 `^[0-9a-fA-F]{64}[[:space:]]+[^[:space:]]+$`，
    **並且**再用 `awk '{print NF}'` 確認欄數恰好是 2（雙保險，Codex 建議的兩種做法都做）。
  - `run-e2e.ps1`：`(\S.*)$` 改成 `(\S+)$`。原本這種行會落到「不在受保護清單內」而被擋下，
    結果雖然也是 FAIL，但訊息會誤導；改完之後兩支腳本都直接判成「格式不合」。
  - 錯誤訊息一併改成「應**恰為**『64 位 hex ＋ 空白 ＋ 路徑』兩欄」。
- **負向測試（8 種情境 × 兩支腳本，全部 exit 1）**：
  ...
  | 少一項 | 1 | 1 |
  | **路徑含空白（第三欄的變形）** | **1** | **1** |
  | 正常 | PASS，7 passed | PASS，7 passed |

  第三欄那一筆用的就是 Codex 報告裡給的原句，兩支腳本現在都印出
  「基準檔第 1 行格式不合（應恰為「64 位 hex ＋ 空白 ＋ 路徑」兩欄）」。
...
|---|---|
| `run-e2e.ps1` 對 solution-app | **7 passed / 0 failed / 0 skipped，exit 0** |
| `run-e2e.sh` 對 solution-app | **7 passed / 0 failed / 0 skipped，exit 0** |
| manifest fail-closed 負向測試 | 8 種情境 × 2 支腳本，全部正確拒絕 |
| 狀態欄 invariant 的事實依據 | 實測：1 顆按鈕、3 個選項、無空白哨兵、重點選不會取消 |
| App 程式碼 | **這一輪 0 變更**（狀態欄選的是不改 App 的那條路） |
| 說法一致性 | 測試名稱、測試註解、HANDBOOK 對照表、本文件，四處都改成「5 欄驗錯 ＋ 狀態進不了空值」 |
