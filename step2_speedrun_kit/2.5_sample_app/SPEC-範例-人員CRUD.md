...
> 用途：以 AI Harness 樣板格式，把本專案「CRUD 標準範本」的人員 CRUD 填成一份**完整示範 SPEC**（教學用「填好的考卷」，非最小雛形）。對應 harness 樣板：`harness/SPEC.md`。
> 來源：`doc/crud-template/SRS-CRUD標準範本-v1.0.md`（FR-T/BR-T）、`SDD-CRUD標準範本-v1.1.md`（設計）。
...
- 沿用 SRS 的 `FR-T-*`（功能）、`BR-T-*`（商業規則）、`NFR-T-*`（非功能）編號；本 SPEC 補 `TC-T-*` 測試案例。
...
由 SRS FR-T 轉為 Given / When / Then；優先級用 MoSCoW。
...
NFR（量化，見 SRS §6）：RWD 斷點 `md`=768px 切換表格/卡片（NFR-T-01）；互動元件皆有 focus-visible ring、icon 鈕有 aria-label（NFR-T-02）；全面 design token 無硬編碼色值（NFR-T-03）；單檔 ≤ 500 行（`[id].vue` 例外上限 550）、無 magic literal/console.log/死碼（NFR-T-04）；衍生資料一律 computed，template 內不執行 O(n) 掃描（NFR-T-05）。
...
SDD §2.3 預留之 RESTful 端點草案（`[SD 待定]`，供後端接軌時定案）：
...
- empty：篩選出空集合 → 空狀態。
- permission denied：`[SA 確認]`（本版無權限控制，見 Permissions）。

## TDD Cases

`[待實機/DEV 實作]`：SRS §7 明列自動化測試框架導入為範圍外，本版以人工目視 + Token Check 驗收。下列為建議案例（對應 FR，供未來導入單元測試時填實）：
...
Existing CRUD example: `frontend/pages/template/crud/`（`index.vue` 列表、`[id].vue` 檢視/編輯/新增三合一）；資料層 `frontend/composables/useTemplateMembers.ts`、列表狀態工廠 `useTemplateListPage.ts`；util `utils/templateValidation.ts`、`utils/templateCsv.ts`；元件 `components/template/TemplateFormField.vue`、`TemplateStatusBadge.vue`。

Design System summary: `design-system-kit/harness/design-system-summary.md`（本專案填好的摘要，等同 harness `modules/crud-ui/design-system-summary.md`）。
...
| Action | Role | Rule |
|---|---|---|
| Create | `[SA 確認]` | 本版無權限控制（SRS §7 範圍外）；未來角色與規則待 SA 確認 |
| Edit | `[SA 確認]` | 同上 |
| Delete | `[SA 確認]` | 同上；刪除仍須二次確認（BR-T-04）|

## Assumptions

- 無後端：資料層為前端 mock，全非同步以預留接縫（SRS 設計原則「後端就緒」）。驗證方式：`useTemplateMembers.ts` 函式簽章與 SDD §2.3 一致。
- 無權限控制與自動化測試（SRS §7 範圍外）。驗證方式：SRS §7 明列。
- 模組屬「平時」類別 → 白/淡色底，不做災時/演練樣式。驗證方式：`design-token.css` 模組底色規則。

## Done Criteria

完工三關見 `HARNESS.md`；本 UI 雛形之 CRUD Mode 條款按實況調整：

- SRS §8 驗收清單逐條成立（UC-T-01~06 全流程、每個 FR/BR 檢核、桌機 1280 / 手機 390 視覺、URL 帶條件重整一致、CLAUDE.md 禁止事項逐條比對）。
...
- 因 SRS §7 排除自動化測試，L1/L2/L3 於本版不適用；驗證以 Token Check + 人工目視為主。
