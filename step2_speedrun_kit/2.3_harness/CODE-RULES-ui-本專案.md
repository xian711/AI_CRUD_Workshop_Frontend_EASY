...
| 樣式 | 依 design token 三層（COMP→SYS→REF）與 `../2.4_design_system/harness/design-system-summary.md`、`../2.4_design_system/`；**禁止**硬編碼 hex/px、禁止引用 REF、禁止 `<style scoped>` 蓋 Nuxt UI（改 `app.config.ts` 的 `ui`）、禁止 `!important` |
...
| Design System 摘要 / Token 規則 | `../2.4_design_system/harness/design-system-summary.md` 與 `../2.4_design_system/`（token 檢查）|
