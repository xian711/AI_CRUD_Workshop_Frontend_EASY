...
路由：後端 API → /api-loop｜後端＋畫面 → /crud-loop｜純雛形 → /ui-loop｜GIS → 先載 skill gis-frontend｜接手舊系統 → 先跑 templates/intake-checklist.md｜段落收尾 → /milestone-loop｜完工審查（新對話）→ /review-loop
