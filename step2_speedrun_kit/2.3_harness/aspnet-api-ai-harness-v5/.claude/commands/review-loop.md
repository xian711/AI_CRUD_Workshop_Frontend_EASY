...
審查閘門。必須在**非產碼的新對話**執行（審寫分離）。審查者只指出問題，不改程式碼。建議以 Opus 執行本 loop（審查判斷力優先於成本）；非 Opus 時先提醒一句再繼續，不阻塞。
...
10. 申辯重審：修正方對單一 ❌ / ⚠️ 附依據（SPEC 條款、實測結果）申請重審時，只重審該條、只對照該依據，不重開全案；每條限一次。誤判 → 撤回該條；維持原判 → 修正方照改，或以選擇題升級問人。
