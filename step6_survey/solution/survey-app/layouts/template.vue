...
// 選單只有 2 項：填寫問卷（填答頁）／回覆管理（主辦方看全部回覆）。
// 本站只有問卷兩頁。範本原本的人員 CRUD 頁已移除——問卷用不到，留著只會變成死碼。
const navLinks = [
  { to: '/survey', label: '填寫問卷', icon: 'i-heroicons-pencil-square' },
  { to: '/survey/admin', label: '回覆管理', icon: 'i-heroicons-table-cells' },
] as const

// 目前路由高亮：精確命中優先。有更精確的選單項命中時（例：/survey/admin），
// 就不把子路徑再算給上層（/survey），避免兩項同時高亮。
function isActive(to: string): boolean {
  if (route.path === to) return true
  const hasExactMatch = navLinks.some(link => link.to === route.path)
  return !hasExactMatch && route.path.startsWith(`${to}/`)
}
...
const pageTitle = computed(
  () => pageTitleOverride.value || (route.meta.pageTitle as string) || '課程回饋問卷',
)
