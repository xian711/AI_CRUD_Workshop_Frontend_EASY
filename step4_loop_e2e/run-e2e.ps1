...
$AppMarker = 'disaster-color-mode'  # solution-app 的 colorMode.storageKey 前綴，用來確認 3100 是本 App
...
try {
    ...
    if (-not $alive) {
        Write-Host "FAIL：solution-app 沒在 $Port 埠回應。" -ForegroundColor Red
        Write-Host '請先啟動 solution-app：cd ../step3_new_module/solution-app; pnpm dev' -ForegroundColor Yellow
        exit 1
    }
    if (-not $hasMarker) {
        Write-Host "FAIL：$Port 埠有回應，但不是本 App（可能被別的程式佔用）。" -ForegroundColor Red
        Write-Host '請關掉佔用的程式，改在 3100 啟動 solution-app。' -ForegroundColor Yellow
        exit 1
    }
    Write-Host "PASS：solution-app 在 $Port 埠、確認為本 App。" -ForegroundColor Green
